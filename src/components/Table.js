...
export default function Table(props) {
    ...
    function Row(rowProp) {
        return (
            selectedHacker.email === rowProp.hacker.email ? (
                <SelectedRowDiv onClick={() => selectHacker(rowProp.hacker)}>
                    <div style={styles.nameEmailContainer}>
                        <SelectedName>{rowProp.hacker.fname} {rowProp.hacker.lname}</SelectedName>
                ...
                <UnselectedRowDiv onClick={() => selectHacker(rowProp.hacker)}>
                    <div style={styles.nameEmailContainer}>
                        <UnselectedName>{rowProp.hacker.fname} {rowProp.hacker.lname}</UnselectedName>
    ...
    return (
        <div style={{"display": "flex"}}>
            <div style={{"flex": 1}}>
                {props.hackers.map((hacker, index) => {
                    return (
                            <Row key={hacker.email} hacker={hacker} index={index}/>
                        )
                    })
                }
            </div>
